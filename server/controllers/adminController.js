...
export const getAllBlogsAdmin=async(req,res)=>{
    try {
        ...
        const blogs=await  Blog.find({}).sort({createdAt:-1});//to get all blogs in descending order
...
export const gettAllComments=async(req,res)=>{
    try {
        const comments=await Comment.find({}).populate('blog').sort({createdAt:-1});//to get all comments in descending order
